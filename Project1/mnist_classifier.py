...
def main():
    ...
    # model fitting
    model = MyLogisticReg([False, X_train, y_train, X_test, y_test, X, y])
    #model = MyLosgisticReg()
    #classifier.fit(w)
    #pikle.dump(model,open("mnist_classifier.kpl","wb"))
    ...
    options = [False, model.X_train, model.y_train, model.X_test, model.y_test, X, y]
